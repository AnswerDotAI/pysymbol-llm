...
- ` def store_attr(names, self, but, cast, store_args)`
...
- ` def argwhere(iterable, f, negate)`
    Like `filter_ex`, but return indices for matching items

- ` def filter_ex(iterable, f, negate, gen)`
    Like `filter`, but passing `kwargs` to `f`, defaulting `f` to `noop`, and adding `negate` and `gen`

- ` def renumerate(iterable, start)`
    Same as `enumerate`, but returns index as 2nd element instead of 1st

- ` def first(x, f, negate)`
...
- `@delegates(_docments) def docments(elt, full)`
    Generates a `docment`
...
- ` class L`
    ...
    - ` def map(self, f)`
    - ` def argwhere(self, f, negate)`
    - ` def argfirst(self, f, negate)`
    - ` def filter(self, f, negate)`
    - ` def enumerate(self)`
    - ` def renumerate(self)`
...
- ` def urlread(url, data, headers, decode, return_json, return_headers, timeout)`
...
- ` def do_request(url, post, headers)`
...
- ` def http_response(body, status, hdrs)`
...
- ` class ThreadPoolExecutor`
    ...
    - ` def __init__(self, max_workers, on_exc, pause)`
...
- `@delegates() class ProcessPoolExecutor`
    ...
    - ` def __init__(self, max_workers, on_exc, pause)`
    - ` def map(self, f, items)`
- ` def parallel(f, items)`
    Applies `func` in parallel to `items`, using `n_workers`

- ` def run_procs(f, f_done, args)`
    Call `f` for each item in `args` in parallel, yielding `f_done`

- ` def parallel_gen(cls, items, n_workers)`
...
- ` class Func`
    ...
    - ` def __repr__(self)`
    - ` def __call__(self, t)`
- ` def compose_tfms(x, tfms, is_enc, reverse)`
...
- ` class FT`
    ...
    - ` def __init__(self, tag, cs, attrs, void_)`
    - ` def __setattr__(self, k, v)`
    - ` def __getattr__(self, k)`
...
- `@contextmanager def maybe_open(f, mode)`
    Context manager: open `f` if it is a path (and close on exit)

- ` def mkdir(path, exist_ok, parents, overwrite)`
...
- `@contextmanager def modified_env()`
